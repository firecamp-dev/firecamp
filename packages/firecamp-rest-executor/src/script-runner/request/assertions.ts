...
export default function (chai: Chai.ChaiStatic, utils: Chai.ChaiUtils) {
    ...
    utils.addMethod(Assertion.prototype, 'url', function (url: IUrl) {
        this.assert(
    ...
    utils.addMethod(Assertion.prototype, 'method', function (method: EHttpMethod) {
        this.assert(
    ...
    utils.addMethod(Assertion.prototype, 'query', function (queryName: string) {
        this.assert(
            !_array.isEmpty(this._obj?.url?.query_params) && this._obj?.url?.query_params?.find(query => query.key === queryName),
            `expecting request should have query ${queryName} but not found`,
            `expecting request should not have query ${queryName}`,
            queryName,
            this._obj?.url?.query_params
        )
    })

    utils.addMethod(Assertion.prototype, 'header', function (headerName: string) {
        this.assert(
            !_array.isEmpty(this._obj?.headers) && this._obj.headers?.find(header => header.key === headerName),
    ...
    utils.addMethod(Assertion.prototype, 'body', function (contentType: ERestBodyTypes) {
