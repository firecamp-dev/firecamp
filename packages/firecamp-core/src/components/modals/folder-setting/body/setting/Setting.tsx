...
const Setting: FC<ISetting> = ({
  ...
  useEffect(() => {
    if (propDescription !== state.description) {
      setState((ps) => {
        return { ...ps, description: propDescription };
      });
    }
  }, [propDescription]);
  ...
  let _validateName = (name = '') => {
    ...
    if (name.length < 1) {
      setError({
        ...
        hasError: true,
        message:
          'The module name is required and It must contain at least 1 character',
      });
      return false;
    }
