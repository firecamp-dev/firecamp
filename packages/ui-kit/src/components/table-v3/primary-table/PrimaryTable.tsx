import { FC, MutableRefObject, ReactNode, createRef, useCallback, useEffect, useRef, useState } from 'react';
...
const Table: FC<ITable<any>> = ({
    ...
    const row1Id= nanoid(), row2Id = nanoid(); // show two empty row on load
    const defaultRows= showDefaultEmptyRows? {[row1Id]: { id: row1Id, ...defaultRow}, [row2Id]: { id: row2Id, ...defaultRow } }: {};
    ...
    const onChangeCell: TOnChangeCell = (cellKey: string, cellValue: any, rowId: string, e: any)=> {
        _setRows((rws)=> {
            const rows = {
                ...rws,
    ...
    return (
        <div className={"w-full"}>
            <table className="primary-table border border-appBorder mb-4" style={{minWidth: "500px"}} ref={tableRef}>
                <thead>
                    <Tr className="border text-base text-left font-semibold bg-focus2">
                        {
                            columns.map((c, i)=> {
                                return <Th style={{ width: c.width }} key={i}> { renderColumn(c) } </Th>
                            })
                        }
                    </Tr>
                </thead>
...
interface IColumn {
    id: string, name: string, key: string, width?: string
}
...
type TRenderCell<R> = (column: IColumn, cellValue: any, index: number, row: R, tableApi: TTableApi, onChange: (ck: string, cv: any, e: any)=> void)=> ReactNode
type TOnChangeCell = (cellKey: string, cellValue: any, rowId: string, e: any)=> void
