...
const Input: FC<IInput> = React.forwardRef(({
    ...
    return (
            <div className={cx("relative items-center text-inputText text-sm w-full mb-5",wrapperClassName)} >
            {label !== '' &&    <label className="text-appForeground text-sm mb-1 block" htmlFor={label}>{label}</label>}
            {
                !!(icon) &&
                <span className={
                    cx('absolute flex items-center top-2',
                        { 'left-3': hasIconLeft },
            ...
            {isEditor === false ?
                ...
                <div className={cx("w-full relative",{ 'flex' : postComponents != '' })}>
                    <input
                        {...domProps}
                        ref={ref}
                        id={id}
                        ...
                        type={type}
                        className={
                            cx('border !border-inputBorder rounded-sm p-2 leading-5 outline-none placeholder-inputPlaceholder text-base focus:bg-inputFocusBackground w-full bg-inputBackground',
                                { '!pl-9': hasIconLeft },
                                { '!pr-9': hasIconRight },
                                className
                            )}
                        ...
                        tabIndex={1}
                />
                <span className={cx("absolute top-3 cursor-pointer",
                { 'left-2': hasIconLeft },
...
const Note = ({ note = '' }) => {
    return (<div className="text-xs text-appForeground  flex items-center">
        <VscInfo />
        {note}
    </div>)
}
