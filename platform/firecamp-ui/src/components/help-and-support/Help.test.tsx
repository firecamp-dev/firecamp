...
describe("Help & Support component", () => {
    ...
    test("validating the default links ", () => {
        ...
        expect(allLink[1]).toHaveAttribute('href', 'https://github.com/firecamp-dev/firecamp/issues/new?assignees=&labels=&template=bug_report.md&title=[http]%20Title%20or%20Feature%20request');
    ...
    test("validating the dynamic title to raise a new github issue", () => {
        ...
        const allLink = screen.getAllByRole('link');
        expect(allLink[1]).toHaveAttribute('href', `https://github.com/firecamp-dev/firecamp/issues/new?assignees=&labels=&template=bug_report.md&title=[${HelpComponentArgs.client}]%20Title%20or%20Feature%20request`);
