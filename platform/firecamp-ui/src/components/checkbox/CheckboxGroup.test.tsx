...
import { CheckboxGroup, ICheckbox } from '@firecamp/ui';
...
const TemplateWithState = (args: any) => {
  ...
  return (
    <CheckboxGroup
      ...
      list={checkboxList}
      onToggleCheck={(value) => {
        let Index = checkboxList.findIndex(
          (list: ICheckbox) => list.id == Object.keys(value)[0]
        );
        let updatedList = checkboxList.map((list: ICheckbox, index: number) =>
          index !== Index
            ? list
            : { ...list, isChecked: Object.values(value)[0] }
        );
...
describe('CheckboxGroup: ', () => {
  const mountCheckBoxGroup = (args = {}) =>
    render(
      <TemplateWithState {...Object.assign({}, CheckBoxPrimaryArg, args)} />
    );
  ...
  test('should render & update on showLabel arg', async () => {
    ...
    expect(CheckboxGroupWrapper.firstElementChild.textContent).toBe(
      CheckBoxPrimaryArg.checkboxLabel + ': '
    );
    expect(CheckboxGroupWrapper.firstElementChild).toHaveClass(
      'cursor-pointer'
    );

    let CheckboxListWrapper =
      CheckboxGroupWrapper.lastElementChild.firstElementChild;

    expect(CheckboxListWrapper).toHaveClass('flex');
    expect(CheckboxListWrapper.childElementCount).toBe(
      CheckBoxPrimaryArg.list.length
    );

    unmount();

    //validate in case showLabel is false
    let { container: containerWithoutLabel } = mountCheckBoxGroup({
      showLabel: false,
    });
    {
  ...
  test('should update based check value', async () => {
    ...
    let lastCheckbox = CheckboxListWrapper.firstElementChild
