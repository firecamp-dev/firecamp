...
export interface IWorkspaceStore {
  ...
  moveFolder: (
    folderId: TId,
    to: { position?: number; collectionId: string; folderId?: string }
  ) => Promise<any>;
  moveRequest: (
    requestId: TId,
...
export const useExplorerStore = create<IWorkspaceStore>((set, get) => ({
  ...
  fetchExplorer: async (workspaceId) => {
    ...
    const wId = workspaceId || useWorkspaceStore.getState().workspace.__ref.id;
  ...
  openFolderTab: (folderId: TId) => {
    const state = get();
    const folder = state.explorer.folders?.find((f) => f.__ref.id == folderId);
    if (folder) {
